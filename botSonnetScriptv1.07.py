...
class BinanceScalpingBot:
    ...
    def log_position_status(self):
        """Log simplificado usando solo datos oficiales de Binance API"""
        try:
            # Obtener posición actual
            current_position = self.get_current_position()

            if not current_position:
                LoggingHelper.log_subsection_header("📊 SIN POSICIONES ACTIVAS")
                return

            # Extraer datos oficiales de Binance
            ...
            percentage = float(current_position['percentage'])
            leverage = float(current_position['leverage'])
            notional = float(current_position['notional'])
            initial_margin = float(current_position['initialMargin'])
            ...
            # Log estructurado y limpio
            LoggingHelper.log_subsection_header("📊 ESTADO DE POSICIÓN (BINANCE API)")
            logger.info(f"   🔄 Dirección: {position_direction}")
            logger.info(f"   📊 Tamaño: {position_size} {self.symbol.replace('USDT', '')}")
            logger.info(f"   💰 Precio entrada: ${entry_price:.4f}")
            logger.info(f"   🏷️ Precio marca: ${mark_price:.4f}")
            logger.info(f"   💵 PnL: ${unrealized_pnl:.4f} USDT")
            logger.info(f"   📈 ROI: {percentage:.2f}%")
            logger.info(f"   ⚖️ Apalancamiento: {leverage:.0f}x")
            logger.info(f"   💎 Nocional: ${notional:.2f} USDT")
            logger.info(f"   💼 Margen inicial: ${initial_margin:.4f} USDT")
            logger.info(f"   🏦 Modo: {position_side}")

            # Información de SL/TP si está registrada
            if self.position:
                ...
                # Calcular distancias usando precio de marca
                if self.position['side'] == 'LONG':
                    sl_distance = ((mark_price - self.position['stop_price']) / mark_price) * 100
                    tp_distance = ((self.position['tp_price'] - mark_price) / mark_price) * 100
                else:  # SHORT
                    sl_distance = ((self.position['stop_price'] - mark_price) / mark_price) * 100
                    tp_distance = ((mark_price - self.position['tp_price']) / mark_price) * 100

                logger.info(f"   📏 Distancia a SL: {sl_distance:+.2f}%")
                logger.info(f"   📏 Distancia a TP: {tp_distance:+.2f}%")
            ...
            if open_orders:
                ...
                for order in open_orders:
                    ...
                    logger.info(f"      • {order_type} {side} @ ${price}")
            else:
                logger.info(f"   📋 Órdenes abiertas: 0")
    ...
    def process_new_kline(self, kline_data: Dict):
        ...
        try:
            ...
            current_position = self.get_current_position()
            if current_position:
                # Solo mostrar estado básico durante el procesamiento de velas
                position_amt = float(current_position['positionAmt'])
                unrealized_pnl = float(current_position['unRealizedProfit'])
                percentage = float(current_position['percentage'])
                position_direction = "SHORT" if position_amt < 0 else "LONG"
                ...
                logger.info(
                    f"📊 Posición activa: {position_direction} | PnL: ${unrealized_pnl:.4f} USDT ({percentage:.2f}%)")
...
if __name__ == "__main__":
    ...
    LoggingHelper.log_section_header("INICIANDO PROGRAMA DE TRADING BOT CRAKEN", "🚀")
