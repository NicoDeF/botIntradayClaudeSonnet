...
class BinanceScalpingBot:
    """Bot principal de scalping para SOL en Binance Futures"""
    ...
    def on_kline_message(self, ws, message):
        """Maneja mensajes de WebSocket de klines"""
        try:
    ...
    def process_new_kline(self, new_kline: Dict):
    ...
    def start_websocket(self):
        """Inicia el WebSocket para datos en tiempo real"""
        socket_url = f"wss://stream.binance.com:9443/ws/{self.symbol.lower()}@kline_{self.timeframe}"
        if self.testnet:
            socket_url = f"wss://stream.binancefuture.com/ws/{self.symbol.lower()}@kline_{self.timeframe}"

        self.ws = websocket.WebSocketApp(
            socket_url,
            on_message=self.on_kline_message,
            on_error=lambda ws, error: logger.error(f"WebSocket error: {error}"),
            on_close=lambda ws, close_status_code, close_msg: logger.info("WebSocket cerrado"),
            on_open=lambda ws: logger.info("WebSocket conectado")
        )
        ...
        try:
            while self.is_running:
                ...
                if current_pos:
                    ...
                    logger.info(f"Posición actual: {current_pos['side']} | PnL: {pnl:.4f} USDT")
